...
func buildSSRURLMain(host, port, protocol, method, obfs, password string) string {
...
func TestGetProxyDetailsFromServerJSON_SSR(t *testing.T) {
	ssrURL := buildSSRURLMain("127.0.0.1", "16276", "origin", "aes-256-cfb", "plain", "testpassword")

	router, err := getRouter(true)
	require.NoError(t, err)

	body := bytes.NewBuffer([]byte(fmt.Sprintf(`{"address":"%s","timeout":30}`, ssrURL)))
	req, _ := http.NewRequest("POST", "/v2/test", body)
	req.Header.Set("Content-Type", "application/json")
